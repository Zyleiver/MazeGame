	...
	if (page_stage != MAIN_PAGE && page_stage != CHOSEMAP_PAGE) // 画地图
	{
		...
		for (imap = 0; imap <= 2 * yscale; imap++)
		{
			for (jmap = 0; jmap <= 2 * xscale; jmap++)
			{
				...
				{
					DrawCoin(x0 + (jmap - 1) / 2 * length, y0 + (imap - 1) / 2 * length, length, 1);
				}
...
void DrawCoin(double mx, double my, double l, int InMap)
{
	...
	SetPenColor("Yellow");
	MovePen(mx + l * 7 / 8, my + l / 2);
	StartFilledRegion(1);
	DrawArc(l * 3 / 8, 0, 360);
	EndFilledRegion();
	SetPenColor("Black");
	DrawArc(l * 3 / 8, 0, 360);
	MovePen(mx + l / 4+l/28, my + l * 7 / 8-l/28-l/64);
	DrawLine(l / 4-l/28, -l / 4+l/28);
	DrawLine(l / 4-l/28, l / 4-l/28);
	MovePen(mx + l / 4+l/28, my + l * 5 / 8-l/64);
	DrawLine(l / 2-l/28*2, 0);
	MovePen(mx + l / 4+l/28, my + l * 3 / 8);
	DrawLine(l / 2-l/28*2, 0);
	MovePen(mx + l / 2, my + l * 5 / 8-l/64);
	DrawLine(0, -l * 11 / 24+l/32);
}
